/**
 * Первичное наполнение боевой базы: стадии воронки, услуги и одна учётная
 * запись администратора. Запускается при каждом старте контейнера, но
 * данные не перетирает — только досоздаёт недостающее.
 *
 * Обычный JS, без TypeScript: в рантайм-образе нет tsx.
 */
...
async function main() {
  ...
  if ((await prisma.user.count()) === 0) {
    ...
    if (!email || !password) {
      throw new Error(
        'База пуста, а ADMIN_EMAIL/ADMIN_PASSWORD не заданы — войти в CRM будет нечем',
      );
    }
